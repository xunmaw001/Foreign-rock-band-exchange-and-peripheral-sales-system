    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="帖子内容" prop="tiezineirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.tiezineirong" label="帖子内容" prop="tiezineirong">
                    <span v-html="ruleForm.tiezineirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.tiezineirong = this.ruleForm.tiezineirong.replace(reg,'../../../ssm2avzt/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tiezixinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tiezixinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
